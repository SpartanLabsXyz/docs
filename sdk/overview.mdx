---
title: Python SDK
description: Python SDK reference — methods, data classes, and environment variables.
icon: python
---
...
# Or set SIMMER_API_KEY in your environment
client = SimmerClient()
```
...
For external wallets, the server can't sign redemptions for you. Call `auto_redeem()` in your agent's loop to claim winnings from resolved markets.

### Kalshi trading

```python
client = SimmerClient(api_key="sk_live_...", venue="kalshi")
# SOLANA_PRIVATE_KEY is auto-detected from env

result = client.trade(market_id="uuid", side="yes", amount=10.0)
```

Requires SOL for fees, USDC on Solana mainnet, and KYC at [dflow.net/proof](https://dflow.net/proof). See [Kalshi wallet setup](/wallets#kalshi-wallet-solana) for details.
